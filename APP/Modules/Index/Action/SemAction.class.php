...
Class SemAction extends Action
{
  ...
  public function regSempost()
  {
    if (IS_AJAX) {
      ...
      $parent_id = M('member')->where(array('inviteCode' => $parent_id))->getField('id');
      ...
      $idcardzz = validateIdCard($idcard);
      if ($idcardzz == true) {
        $this->ajaxReturn(array('result' => 0, 'info' => '身份证号码格式不正确!'));
      }
  ...
  public function send_sms_reg_code()
  {
    ...
    if (M('member')->where(array('username' => $mobile))->getField('id')) {
