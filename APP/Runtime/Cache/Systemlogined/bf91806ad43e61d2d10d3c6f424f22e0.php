...
<script type="text/javascript">
	$(function() {
				...
				<div id="page-content" class="clearfix">
					...
					<div class="row-fluid">
						<!--PAGE CONTENT BEGINS HERE-->
							<form class="form-horizontal" name="edituser" action="<?php echo U(GROUP_NAME.'/Rbac/editUserHandle');?>" method="post">
								<div class="control-group">
									<label class="control-label" for="username"> 用户名</label>

									<div class="controls">
										<input type="text" value="<?php echo ($user["username"]); ?>" name="username" id="username" readonly="readonly" placeholder="" />
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="password" id="password2" name="password2" placeholder="" /> <span class="help-inline" id="msg"></span>
									</div>
								</div>
								<?php if($user["username"] != C("RBAC_SUPERADMIN")): ?><div class="control-group">
									<label class="control-label" for="role">所属角色</label>
									<div class="controls">
											<?php if(is_array($role)): foreach($role as $key=>$v): ?><input name="role[]" type="checkbox" <?php if(is_array($user["role"])): foreach($user["role"] as $key=>$value): if($v["name"] == $value["name"]): ?>checked="checked"<?php endif; endforeach; endif; ?> value="<?php echo ($v["id"]); ?>"/><span class="lbl"> <?php echo ($v["remark"]); ?></span>&nbsp;&nbsp;<?php endforeach; endif; ?>	
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="radio" <?php if($user["lock"] == 0): ?>checked="checked"<?php endif; ?> value="0" name="lock">
										<span class="lbl"> 启用</span>
										<input type="radio" <?php if($user["lock"] == 1): ?>checked="checked"<?php endif; ?> value="1" name="lock">
		...
		<!--basic scripts-->
		<script type='text/javascript'>
			var checkUserName = "<?php echo U(GROUP_NAME .'/Rbac/checkUserName');?>";
		</script>
