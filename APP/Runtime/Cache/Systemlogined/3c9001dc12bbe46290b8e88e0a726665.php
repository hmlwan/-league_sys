				...
				<div id="page-content" class="clearfix">
					<div class="page-header position-relative">
						<div style="text-align:left;width:100%;">
						    <a type="button" id="clear-log" class="btn btn-info btn-small no-border"> <i class="icon-bolt  icon-white"></i> 清空日志</a>
						</div>
					</div><!--/.page-header-->

					<div class="row-fluid">
						<!--PAGE CONTENT BEGINS HERE-->
						<form id="table-searchbar" method="POST" action="<?php echo U(GROUP_NAME.'/System/systemLog');?>" class="form-inline well well-small">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
											<td><?php echo ($v["id"]); ?></td>
											<td><?php echo ($v["logaccount"]); ?></td>
											<td><?php echo ($v["logdesc"]); ?></td>
											<td><?php echo ($v["logtype"]); ?></td>
											<td><?php echo ($v["logip"]); ?></td>
											<td><?php echo ($v["logiplocal"]); ?></td>
			...
			$(function() {
				...
				$('#clear-log').click(function(){
					bootbox.confirm("确认要清空日志吗?", function(result) {
						if(result) {
							var url = '<?php echo U(GROUP_NAME .'/System/deleteLog');?>';
							window.location = url;
						}
					});
				});
