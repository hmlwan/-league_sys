				...
				<div id="page-content" class="clearfix">
					<div class="page-header position-relative">
						<div style="text-align:left;width:100%;">
						    <a type="button" href="<?php echo U(GROUP_NAME.'/Application/adddown');?>" class="btn btn-small btn-info no-border"> <i class="icon-plus icon-white"></i> 添加下载配置</a>
						</div>
					</div><!--/.page-header-->
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($ann)): foreach($ann as $key=>$v): ?><tr>
											...
											<td><a href="<?php echo ($v["logo_url"]); ?>"><img src="<?php echo ($v["logo_url"]); ?>" style="width: 40px;height: 40px" alt=""></a></td>
											<td><?php echo ($v["size"]); ?></td>
											<td><?php echo ($v["score"]); ?></td>
											<td><?php echo ($v["person_num"]); echo ($v["person_num_unit"]); ?></td>
											<td><?php echo ($v["install_num"]); echo ($v["install_num_unit"]); ?></td>
											<td><?php echo ($v["android_url"]); ?></td>
											<td><?php echo ($v["ios_url"]); ?></td>
											<td>
												<?php switch($v["status"]): case "1": ?>开启<?php break;?>
		...
		<script type="text/javascript">
			$(function() {
				...
				$('.btn-info').click(function(){
					...
					var url = '<?php echo U(GROUP_NAME .'/Application/editdown');?>'+'/id/'+ aid;
					window.location = url;
				});
				...
				$('.btn-danger').click(function(){
					...
					bootbox.confirm("确认删除吗?", function(result) {
						if(result) {
							var url = '<?php echo U(GROUP_NAME .'/Application/deleteDown');?>'+'/id/'+ aid;
							window.location = url;
						}
					});
