				...
				<div id="page-content" class="clearfix">
					<div class="page-header position-relative">
						<div style="text-align:left;width:100%;">
						    <a type="button" href="<?php echo U(GROUP_NAME.'/Counsel/add');?>" class="btn btn-small btn-info no-border"> <i class="icon-plus icon-white"></i> 添加资讯</a>
						</div>
					</div><!--/.page-header-->
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($ann)): foreach($ann as $key=>$v): ?><tr>
											...
											<td class="hidden-480">
												<?php switch($v["type_id"]): case "1": ?>热门咨询<?php break;?>
													<?php case "2": ?>热门交易<?php break;?>
													<?php case "3": ?>:热门项目<?php break; endswitch;?>
											</td>
											<td><?php echo ($v["summary"]); ?></td>
											<td><?php echo ($v["operator"]); ?></td>
											<td><a href="<?php echo ($v["logo_url"]); ?>"><img src="<?php echo ($v["logo_url"]); ?>" style="width: 80px;height: 80px" alt=""></a></td>
											<td><?php echo ($v["sort"]); ?></td>
											<td><?php echo ($v["jump_url"]); ?></td>
		...
		<script type="text/javascript">
			$(function() {
				...
				$('.btn-info').click(function(){
					...
					var url = '<?php echo U(GROUP_NAME .'/Counsel/edit');?>'+'/id/'+ aid;
					window.location = url;
				});
				...
				$('.btn-danger').click(function(){
					...
					bootbox.confirm("确认删除吗?", function(result) {
						if(result) {
							var url = '<?php echo U(GROUP_NAME .'/Counsel/delete');?>'+'/id/'+ aid;
							window.location = url;
						}
					});
