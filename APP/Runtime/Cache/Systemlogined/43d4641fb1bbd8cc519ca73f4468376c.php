							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
											...
											<td><?php if($v["level"] == 0): ?>普通会员<?php else: ?>城主<?php endif; ?></td>
											<td><a target="_blank" href="<?php echo U('inMember',array('u'=>$v['username']));?>"><?php echo ($v["username"]); ?></a></td>
											...
											<td><?php if($v["lock"]): ?><a onclick="if(confirm('确认解封此账户吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/editFeng',array('id'=>$v['id'],'lock'=>0));?>">解封</a>
												...
												<a onclick="if(confirm('确认删除此账户吗?')==false)return false;" href="<?php echo U(GROUP_NAME .'/Member/deleteMember',array('id'=>$v['id']));?>">删除</a>
												</td>
