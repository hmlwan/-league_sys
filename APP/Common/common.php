...
function account_log($member, $money, $desc, $jj, $type = 0, $status = 1, $jid, $tgaward)
{
  ...
  $data['queryTime'] = date("Y-m-d", time());
...
function sendNewSMS($mobile, $content,$type=1)
{
//    return array(
//        'ret'=>0,
//        'msg'=>'发送成功'
//    );
    ...
    if($type == 1){
        $ip_request_num = \app\common\entity\Config::getValue('ip_request_num');

        $ip_interval_times = \app\common\entity\Config::getValue('ip_interval_times');
        $ip_str = \think\Request::ip();
        $ip_w[] = ["ip_str",'=',$ip_str];
        $ip_w[] = array('add_time','between',array(strtotime(date("Y-m-d 0:0:0",time())),strtotime(date("Y-m-d 23:59:59",time()))));
        $ip_monitor = \think\Db::table('ip_monitor')->where($ip_w)->order('add_time desc')->select();

        if(count($ip_monitor)>$ip_request_num){
            return array(
                'ret'=>1,
                'msg'=>'今日请求次数超限'
            );
        }
        if((time()-$ip_monitor[0]['add_time']) < $ip_interval_times){
            return array(
                'ret'=>1,
                'msg'=>'请不要频繁请求'
            );
    ...
    $param = [
        'apikey' => "ffab831be504252b955292991f23c35f",
        'mobile' => $mobile,
        'text' => $content,
    ];

    $url = "https://sms.yunpian.com/v2/sms/single_send.json";
    $result = curlPost($param, $url);

    $jsonarr = json_decode($result, true);
    if($result && $jsonarr['code'] == 0){
        if($type == 1){
            //发送ip monitor
            \think\Db::table('ip_monitor')->insert(array(
                'ip_str' => $ip_str,
                'request_times'=>1,
                'add_time' => time(),
            ));
        ...
        return  array(
            'ret'=>1,
            'msg'=>$jsonarr['msg']?$jsonarr['msg']:'发送失败'
        );
    }
}

function getSMSTemplate($type){
    $send_sms_template =  array(
        '3751462'=>"【云链】交易中心有未按时确认的订单，已经被强制放行。请登录APP查看详情。",
        '3751472'=>"【云链】恭喜您发布的买入订单已经匹配成功，请在120分钟内登录平台完成付款。",
        '3751470'=>"【云链】恭喜您！交易中心有卖出订单已被买家标注为已付款，请在120分钟内处理，超时将自动确认。",
        '3751468'=>"【云链】很遗憾，交易中心有被投诉的订单，请在12小时内前往APP查看详情并处理。",
        '3751464'=>'【云链】恭喜您！卖家已标注已放行，请立即前往APP查看余额。',
        '3751466'=>'【云链】很遗憾，交易中心有未按时付款的订单，已经被限制交易。请登录平台处理。',
        '3751460'=>'【云链】尊敬的用户您好！您的验证码是：@，验证码有效时间为10分钟。请勿向任何人包括客服提供验证码。',
    );
